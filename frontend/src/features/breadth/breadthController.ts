// @ts-nocheck
/* Breadth page controller — ported from web/static/breadth.js */
import { Chart, registerables } from "chart.js";
import { fetchJson, showToast, bjDateKey, bjHourNow, loadHealthBadge, IS_READONLY } from "../../lib/api";
import { chartThemeFromCss } from "../../lib/chartTheme";
...
export function mountBreadthController(root: HTMLElement): BreadthController {
  const el = (id: string): HTMLElement | null => root.querySelector(`#${id}`);
  const els = (sel: string): NodeListOf<Element> => root.querySelectorAll(sel);
...
function renderTable(payload) {
  const thead = root.querySelector("#breadthTable thead");
  const tbody = root.querySelector("#breadthTable tbody");
...
function renderStatusCards(payload) {
  const wrap = el("breadthStatusCards");
  if (!wrap) return;
  const status = payload.status || {};
  const thresholds = payload.thresholds || {};
  const latestRow = (payload.rows || [])[0] || null;
  const coverage = payload.coverage || {};
  const coverageInline = el("coverageInline");
  if (coverageInline) {
...
function updateCockpitActiveUi() {
  els(".cockpit-card-link").forEach((el) => {
...
function applyChartLinkage() {
  ...
  els(".chart-card").forEach((card) => card.classList.remove("is-linked"));
  if (link) {
    const canvas = el(link.canvasId);
...
function renderCockpitHelp(payload) {
  const box = el("cockpitHelpContent");
  if (!box) return;
...
function renderPercentileCards(payload) {
  const wrap = el("breadthPercentileCards");
  if (!wrap) return;
...
function renderBreadthConfig(payload) {
  const box = el("breadthThresholdForm");
  if (!box) return;
...
async function saveBreadthConfig() {
  const status = el("breadthConfigStatus");
  const keys = [
  ...
  const thresholds = {};
  keys.forEach((k) => {
    const v = Number(el(`breadthCfg_${k}`)?.value);
...
function setSyncButtonsBusy(busy, label) {
  const refreshBtn = el("refreshBreadthBtn");
  const syncBtn = el("syncBreadthBtn");
  if (refreshBtn) {
...
async function runBreadthSync(full, { auto = false } = {}) {
  const progress = el("syncProgressText");
...
async function loadBreadth(refresh = false) {
  const btn = el("refreshBreadthBtn");
  ...
  try {
    ...
    el("breadthMeta").textContent = `${payload.row_count} rows · showing ${payload.rows.length} · updated ${updatedAt}`;
    el("explainLink").href = payload.notes?.indicators_explain_url || "https://stockbee.blogspot.com/2022/12/market-monitor-scans.html";
  } catch (err) {
...
function init() {
  loadHealthBadge(el("healthBadge")).catch(() => {});
  const widthRange = el("chartWidthRange");
  const widthValue = el("chartWidthValue");
  ...
  el("refreshBreadthBtn")?.addEventListener("click", () => {
    runBreadthSync(false).catch((err) => showToast(err.message, true));
  });
  el("syncBreadthBtn")?.addEventListener("click", () => {
    runBreadthSync(true).catch((err) => showToast(err.message, true));
  });
  el("saveBreadthConfigBtn")?.addEventListener("click", () => {
    saveBreadthConfig().catch((err) => showToast(err.message, true));
  });
  els("#chartRangeGroup button").forEach((btn) => {
    btn.addEventListener("click", () => {
      currentChartDays = Number(btn.dataset.days || 365);
      els("#chartRangeGroup button").forEach((b) => b.classList.remove("active"));
  ...
  if (IS_READONLY) {
    const syncText = el("syncProgressText");
    if (syncText) syncText.textContent = "Read-only · published snapshot";
  }
}
